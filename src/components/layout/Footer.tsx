      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex gap-4">
              <a href={siteConfig.socials.twitter} target="_blank" rel="noreferrer" className="hover:text-zinc-100 transition-colors" aria-label="Twitter link">
                Twitter
              </a>
              <a href={siteConfig.socials.linkedin} target="_blank" rel="noreferrer" className="hover:text-zinc-100 transition-colors" aria-label="LinkedIn link">
                LinkedIn
              </a>
              <a href={siteConfig.socials.instagram} target="_blank" rel="noreferrer" className="hover:text-zinc-100 transition-colors" aria-label="Instagram link">
                Instagram
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm hover:text-zinc-100 transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li className="pt-2 border-t border-zinc-900/60 mt-2">
                ...
                <a href={`mailto:${siteConfig.emails.general}`} className="text-sm hover:text-zinc-100 transition-colors">{siteConfig.emails.general}</a>
              </li>
              <li>
                <span className="block text-[9px] font-bold text-zinc-500 uppercase tracking-widest leading-none mb-1">Support</span>
                <a href={`mailto:${siteConfig.emails.support}`} className="text-sm hover:text-zinc-100 transition-colors">{siteConfig.emails.support}</a>
              </li>
              <li>
                <span className="block text-[9px] font-bold text-zinc-500 uppercase tracking-widest leading-none mb-1">Careers / HR</span>
                <a href={`mailto:${siteConfig.emails.hr}`} className="text-sm hover:text-zinc-100 transition-colors">{siteConfig.emails.hr}</a>
              </li>
              <li>
                <span className="block text-[9px] font-bold text-zinc-500 uppercase tracking-widest leading-none mb-1">Admin</span>
                <a href={`mailto:${siteConfig.emails.admin}`} className="text-sm hover:text-zinc-100 transition-colors">{siteConfig.emails.admin}</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-zinc-900 flex flex-col md:flex-row items-center justify-between gap-4 text-xs">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <Link href="/editorial-policy" className="hover:text-zinc-100 transition-colors">Editorial Policy</Link>
