...
async function readBlogFile() {
  const response = await fetch(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${PATH}?ref=${BRANCH}`, { headers: githubHeaders(), cache: "no-store" });
...
export async function GET() {
  ...
  try { return NextResponse.json(await readBlogFile()); }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load articles" }, { status: 500 }); }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body?.action === "save") {
      const article = body?.article as Record<string, unknown> | null;
      if (!article || typeof article !== "object") return NextResponse.json({ error: "Article payload is required" }, { status: 400 });
      ...
      for (const key of required) {
        ...
        if (typeof value !== "string" || !value.trim()) return NextResponse.json({ error: `${key} is required` }, { status: 400 });
      }
      if (article.status !== "draft" && article.status !== "published") return NextResponse.json({ error: "Status must be draft or published" }, { status: 400 });

      const normalized: Record<string, unknown> = { ...article, featured: Boolean(article.featured), tags: Array.isArray(article.tags) ? article.tags : [] };
      const index = articles.findIndex((item) => item.id === normalized.id);
      if (index >= 0) articles[index] = normalized; else articles.unshift(normalized);
      await writeBlogFile(articles, current.sha, `${index >= 0 ? "Update" : "Create"} blog article: ${String(normalized.title)}`);
      return NextResponse.json({ ok: true, action: index >= 0 ? "updated" : "created" });
    }

    if (body?.action === "delete") {
      if (typeof body.id !== "string" || !body.id) return NextResponse.json({ error: "Article id is required" }, { status: 400 });
